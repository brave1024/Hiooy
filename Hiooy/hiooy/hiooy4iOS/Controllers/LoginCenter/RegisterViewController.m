...
- (void)viewDidLoad
{
    ...
    [self.btnRegister setBackgroundImage:img forState:UIControlStateNormal];
    ...
    //[self.btnVerify setBackgroundImage:<#(UIImage *)#> forState:<#(UIControlState)#>];
...
- (IBAction)getVerifyCode:(id)sender
{
    ...
    // type: 1-注册 2-忘记密码
    [[UserManager shareInstant] userGetVerifyCode:self.txtfieldMobile.text withType:@"1" completion:^(BOOL isSucceed, NSString *message) {
        if (isSucceed)
        {
            self.btnVerify.enabled = NO;
            [self toast:@"验证码发送成功"];
...
//    [[UserManager shareInstant] userRegister:@"king1024" withPassword:@"123456" andEmail:@"zhiyong.xia@rogrand.com" completion:^(BOOL isSucceed, NSString *message) {
//        if (isSucceed) {
//            NSLog(@"注册成功");
...
//    }];
    ...
    [[UserManager shareInstant] userRegister:self.txtfieldMobile.text withPassword:self.txtfieldPwd.text andVerify:self.txtfieldVerify.text completion:^(BOOL isSucceed, NSString *message) {
        [self stopLoading];
        if (isSucceed)
        {
            NSLog(@"注册成功");
        ...
        {
            NSLog(@"注册失败:%@", message);
            NSString *toastStr = [NSString stringWithFormat:@"注册失败:%@", message];
            [self toast:toastStr];
        }
    }];
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    // 账号为手机号
//    else if (textField == _txtfieldMobile) {
//        if (range.location >= PwdMaxLength) {
//            int maxLength = PwdMaxLength;
